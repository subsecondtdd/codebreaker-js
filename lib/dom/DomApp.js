...
module.exports = class DomApp {
      ...
      this._controller
        ...
        .then(result => {
          ...
          if (typeof nextRender !== 'function')
            throw new Error(
              `No such method: ${this.constructor.name}#${result.view}`
            )
  ...
  showMakerGame({ data, subscribe }) {
    const render = ({ data }) => {
      this._rootElement.innerHTML = `
        <div itemscope>
          <h2><span itemscope itemprop="wordLength" itemtype="http://schema.org/Integer">${
            data.wordLength
          }</span> letter game</h2>
          ${
            data.errorMessage
              ? `<div itemscope itemprop="errorMessage" itemtype="http://schema.org/Text">${
                  data.errorMessage
                }</div>`
              : ''
          }
          <div style="display: none" itemscope itemprop="gameId" itemtype="http://schema.org/Text">${
            data.gameId
          }</div>
          <div style="display: block" itemscope itemprop="gameVersion" itemtype="http://schema.org/Integer">${
            data.gameVersion
          }</div>
          <div itemscope itemprop="gameState" itemtype="http://schema.org/Text">${
            data.gameState
          }</div>
          <table itemscope itemprop="guessList" itemtype="http://schema.org/ItemList">
            ...
            ${data.guessList.map(
              guess => `
              <tr itemscope>
                <td itemprop="guess" itemtype="http://schema.org/Text">${
                  guess.guess
                }</td>
                ${
                  guess.points
                    ? `<td itemprop="points" itemtype="http://schema.org/Integer">${
                        guess.points
                      }</td>`
                    : `<td>
                    <div class="row">
                      <form data-command="score" method="POST" >
                        <input name="gameId" type="hidden" value="${
                          data.gameId
                        }"/>
                        <div class="column">
